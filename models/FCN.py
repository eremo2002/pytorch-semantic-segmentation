...
class FCN16(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(dh, dw)        
        pool4 = pool4[:, :, dh:(dh + upscore2.size()[2]), dw:(dw + upscore2.size()[3])]
        upscore16 = self.upscore16(upscore2 + pool4)
...
class FCN8(nn.Module):
    def __init__(self, num_classes):
        ...
        self.score_fr = nn.Conv2d(4096, num_classes, 1)
        self.upscore2 = nn.ConvTranspose2d(num_classes, num_classes, 4, stride=2, bias=False)
        self.upscore2_pool4 = nn.ConvTranspose2d(num_classes, num_classes, 4, stride=2, bias=False)
    ...
    def forward(self, x):
        ...
        dh, dw = (pool4.size()[2] - upscore2.size()[2])//2, (pool4.size()[3] - upscore2.size()[3])//2        
        pool4 = pool4[:, :, dh:(dh + upscore2.size()[2]), dw:(dw + upscore2.size()[3])]
        upscore2_pool4 = self.upscore2_pool4(upscore2 + pool4)
        #print(f'upscore2_pool4 {upscore2_pool4.size()}')


        dh, dw = (pool3.size()[2] - upscore2_pool4.size()[2])//2, (pool3.size()[3] - upscore2_pool4.size()[3])//2
        pool3 = pool3[:, :, dh:(dh + upscore2_pool4.size()[2]), dw:(dw + upscore2_pool4.size()[3])]        
        upscore8 = self.upscore8(upscore2_pool4 + pool3)
        print(f'upscore8 {upscore8.size()}')


        dh, dw = (upscore8.size()[2] - input_h)//2, (upscore8.size()[3] - input_w)//2
        upscore8 = upscore8[:, :, dh:(dh + input_h), dw:(dw + input_w)]
        return upscore8
